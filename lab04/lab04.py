...
class ArrayList:
    ...
    def insert(self, idx, value):
        ...
        ### BEGIN SOLUTION
        normalized = self._normalize_idx(idx)
        if normalized > self.len:
            raise IndexError
        if self.len == len(self.data):
            shifted = ConstrainedList((self.len * 2) + 1)
            for i in range(idx):
                shifted[i] = self.data[i]
            shifted[idx] = value
            for i in range(self.len, idx, -1):
                shifted[i] = self.data[i - 1]
            self.data = shifted
        else:
            for i in range(self.len, idx, -1):
